...
// Endpoint to fetch all user directoriesrouter.get('/userdirectories', async (req, res) => {    try {        const haiiloApiUrl = `https://your-haiilo-instance.com/api/userdirectories`;        const response = await fetch(haiiloApiUrl, {            method: 'GET',            headers: {                'Authorization': `Bearer YOUR_ACCESS_TOKEN`, // Replace with your Haiilo API token                'Content-Type': 'application/json'            }        });        if (!response.ok) {            throw new Error(`Failed to fetch user directories: ${response.statusText}`);        }        const data = await response.json();        res.json(data);    } catch (error) {        console.error('Error fetching user directories:', error);        res.status(500).json({ error: 'Failed to fetch user directories' });    }});module.exports = router;